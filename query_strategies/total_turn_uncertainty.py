...
class TotalTurnUncertainty(Strategy):
    def __init__(self, dataset, net, ignore_top_k_percent: int = 5):
        super(TotalTurnUncertainty, self).__init__(dataset, net)
    ...
    def query(self, n):
        ...
        aggreagated_uncertainties = [
            np.sum(dialogue_uncertainties) for dialogue_uncertainties in uncertainties
        ]
